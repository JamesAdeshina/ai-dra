...
import { connectedSurvivors } from "@/features/carer/data/carer-dashboard-data";
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <label className="relative block sm:w-[265px]">
        <span className="sr-only">Choose survivor</span>
        <Users
          size={19}
        ...
        >
          <option value="all">
            Viewing: All Survivors ({connectedSurvivors.length})
          </option>
          {connectedSurvivors.map((survivor) => (
